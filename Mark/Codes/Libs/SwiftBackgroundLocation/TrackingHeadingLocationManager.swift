...
public enum LocationResult<T> {
    case Success(T)
    case Error(Error)
}

public typealias LocationManagerListener = (LocationResult<TrackingHeadingLocationManager>) -> ()
public typealias HeadingLocationListener = (LocationResult<LocationHeading>) -> ()
...
final public class TrackingHeadingLocationManager: NSObject {
    ...
    public func manager(for status:LocationAuthorizationStatus, completion: @escaping LocationManagerListener) {
        ...
        if status.isAuthorized(for: CLLocationManager.authorizationStatus()) {
            locationManagerListener?(LocationResult.Success(self))
            return
        }
    ...
    public func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        ...
        if requestedStatus == .always {
            if status == .authorizedWhenInUse {
                ...
                locationManagerListener?(LocationResult.Error(LocationAuthorizationError.cantGetAlways))
            }
        } else {
            locationManagerListener?(LocationResult.Error(LocationAuthorizationError.userDenied))
        }
    }
...
extension TrackingHeadingLocationManager: CLLocationManagerDelegate {
    ...
    public func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        if manager == significantLocationManager {
            ...
            headingListener?(LocationResult.Success(LocationHeading(location: lastLocation, heading: lastHeading)))
        }
    }
    ...
    public func locationManager(_ manager: CLLocationManager, didUpdateHeading newHeading: CLHeading) {
        ...
        self.headingListener?(LocationResult.Success(LocationHeading(location: lastLocation, heading: lastHeading)))
    }
}
