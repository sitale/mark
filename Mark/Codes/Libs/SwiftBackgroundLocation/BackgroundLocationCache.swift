...
struct BackgroundLocationCache: BackgroundLocationCacheable {
    ...
    func coordinates(for region: CLRegion) -> CLLocationCoordinate2D {
        let lat  = defaults.double(forKey: String(format: CacheFormat.latFormat, region.identifier))
        let long = defaults.double(forKey: String(format: CacheFormat.longFormat, region.identifier))
